...
func TestUint64Old(t *testing.T) {
	tests := []struct {
		name string
		in   []uint64
		want []uint64
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			radixsort.Uint64Old(data, buf)

			if !reflect.DeepEqual(tt.want, data) {
				t.Errorf("Unit64Old(%v) = %v, want %v", tt.in, data, tt.want)
			}
		})
	}
}

func TestUint64OldMatchesStdlib(t *testing.T) {
...
func benchmarkUint64Old(b *testing.B, size int, mode string) {
	...
	for b.Loop() {
		...
		radixsort.Uint64Old(tmp, buf)
	}
}

func BenchmarkUint64Old(b *testing.B) {
	for _, size := range sizes {
		for _, mode := range modes {
			b.Run(func() string {
				return fmt.Sprintf("RadixsortUint64Old_%d_%s", size, mode)
			}(), func(b *testing.B) {
				benchmarkUint64Old(b, size, mode)
			})
		}
	}
}
