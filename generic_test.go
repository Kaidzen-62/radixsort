...
/*
	Из того что я прочел в интернете я делаю вывод что для нашего случая следует сделать такие типы тестов:
	- [x] краевые случаи для int8 // тестируем как влияет кол-во байт
  - [x] краевые случаи для int64 // заодно проверим как работают числа со знаком
  - [x] краевые случаи для uint64 // проверяем работает ли базовый алгоритм
  - [x] рандомный массив на 1000 эл-ов для uint64 // всякое говно бывает
  - [x] краевые случаи для float32 // для float64 можно не проверять
  - [ ] краевые случаи для int (хз, но следует проверить)
  - [x] пара простых тестов для структур с сортируемыми полями
*/
